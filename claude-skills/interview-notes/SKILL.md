...
**Filename:** run `date +%Y-%m-%d-%H%M%S` to get `{timestamp}`. Use `{timestamp}-{company-slug}-interview-prep.md`.

Create the logs directory if needed: `mkdir -p ~/fun/adv1k/job-search/logs/`.

Write the log file. After writing, verify the file exists: `ls ~/fun/adv1k/job-search/logs/{filename}`. If the file is not found, report: "Log write failed -- file not found at expected path. Check permissions." Otherwise print: `Log saved -> ~/fun/adv1k/job-search/logs/{filename}`

No em dashes in the log file either.
...
{Step 5 full output -- all sections}
```
